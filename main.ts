...
if (input.pin === pincode) {
  ...
  if (option.options === "Withdraw") {
    ...
    console.log(
      chalk.magentaBright.bold.italic("Your remaining balance is : "),
      chalk.blueBright.bold.italic(myBalance - amount.amount)
    );
  }
  if (option.options === "Check Balance") {
    console.log(
      chalk.magentaBright.bold.italic("Your current balance is : "),
      chalk.greenBright.bold.italic(myBalance)
    );
  }
} else {
